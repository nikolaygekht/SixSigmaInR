#https://support.minitab.com/en-us/minitab/19/help-and-how-to/quality-and-process-improvement/control-charts/how-to/attributes-charts/u-chart/methods-and-formulas/methods-and-formulas/
library(ggplot2)

k = 3;
data <- read.csv("ch23p450.txt", header=TRUE);

epu = data$Errors / data$Calls;


mr = sum(data$Errors) / sum(data$Calls);
mr;
ucl = matrix(0, length(epu));
lcl = matrix(0, length(epu));

for (i in seq(1, length(epu))) {
    v = k * sqrt(mr / data$Calls[i]);
    ucl[i] = mr + v;
    lcl[i] = mr - v;
    if (lcl[i] < 0) {
        lcl[i] = 0;
